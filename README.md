...
```
             ...
             ┌─────────────────────┐
             │      planner         │  测试计划 + 执行计划
             │    (LLM 节点)        │  (含 YAML execution_plan)
             └──────────┬──────────┘
                        │
                        ▼
             ┌─────────────────────┐
             │     generator        │  代码生成 + 安全审查
             │    (LLM 节点)        │  → 保存至 test_case/
             └──────────┬──────────┘
                        │
                        ▼
             ┌─────────────────────┐
             │  sandbox_executor    │  沙盒执行 (策略模式)
             │   (Docker/SSH)       │  成功 → 结束
             └──────────┬──────────┘  失败 → 回到 planner (最多 3 轮)
                        │
              ┌─────────┴─────────┐
              ▼                   ▼
            成功 ✅            失败 → planner (修复循环)
...
**注**: `context_retriever`（RAG 上下文检索）暂时注释，可在需要时启用，用于减少 API 调用延迟。
